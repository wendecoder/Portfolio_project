...
def show_venue(venue_id):
  ...
  venues = db.session.query(Show).join(Artist).filter(venue_id == venue_id)
  data={
    "id": venue.id,
    "name": venue.name,
    "genres": venue.genres,
  ...
  paa = {}
  for n in range(i):
    artist_id = venues[n].artist_id;
    artist = Artist.query.get(artist_id)
    pas = {}
    passs = {}
    if venues[n].start_time.replace(tzinfo=None) < datetime.now():
      pas= {"past_shows": [{
        "artist_id": artist.id,
        "artist_name": artist.name,
        "artist_image_link": artist.image_link,
        "start_time": str(venues[n].start_time)
        }]}
    if venues[n].start_time.replace(tzinfo=None) > datetime.now():
      passs = {"upcoming_shows":[{
	"artist_id": artist.id,
	"artist_name": artist.name,
	"artist_image_link": artist.image_link,
	"start_time": str(venues[n].start_time)
...
def show_artist(artist_id):
  ...
  artist = Artist.query.get(artist_id)
  artists = db.session.query(Show).join(Venue).filter(artist_id == artist_id)
  data={
    "id": artist.id,
    "name": artist.name,
  ...
  paa = {}
  for n in range(i):
    venue_id = artists[n].venue_id;
    venue = Venue.query.get(venue_id)
    pas = {}
    passs = {}
    if artists[n].start_time.replace(tzinfo=None) < datetime.now():
      pas= {"past_shows": [{
        "venue_id": venue.id,
        "venue_name": venue.name,
        "venue_image_link": venue.image_link,
        "start_time": str(artists[n].start_time)
        }]}
    if artists[n].start_time.replace(tzinfo=None) > datetime.now():
      passs = {"upcoming_shows":[{
        "venue_id": venue.id,
